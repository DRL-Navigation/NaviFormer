...
class NaviDataset(Dataset):
    ...
    def __data_to_exp(cls, data):
        ...
        index = random.randint(0, len(exp['laser'])-2)
        for key in ['laser', 'vector', 'discrete_action', 'reward']:
            exp['next_'+key] = exp[key][index+1]
        for key in ['laser', 'vector', 'discrete_action', 'reward']:
...
class NaviCollateFN:
    ...
    def collate_fn(self, batch):
        exp_batch = {'laser':[], 'vector':[], 'discrete_action':[], 'reward':[], 'next_laser':[], 'next_vector':[]}
        ...
        return exp_batch['laser'], exp_batch['vector'], exp_batch['discrete_action'], exp_batch['reward'], exp_batch['next_laser'], exp_batch['next_vector']