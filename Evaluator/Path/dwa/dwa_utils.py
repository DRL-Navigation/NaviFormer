...
class DWAPolicy4Nav:
    ...
    def gen_path(self, laser, goal, robot_pose):
        cfg = RobotProp()
        dt = cfg.dt
        T = cfg.T
        action = self.gen_action(laser, goal)[0]
        path = [robot_pose]
        for i in range(int(T//dt)):
            path_i = copy.deepcopy(path[i])
            path_i[2] = path[i][2] + action[1] * dt
            if action[1] != 0:
                path_i[0] = path[i][0] + action[0]/action[1] * (math.sin(path_i[2])-math.sin(path[i][2]))
                path_i[1] = path[i][1] + action[0]/action[1] * (math.cos(path_i[2])-math.cos(path[i][2]))
            else:
                path_i[0] = path[i][0] + action[0] * math.cos(path[i][2]) * dt
                path_i[1] = path[i][1] + action[1] * math.sin(path[i][2]) * dt
            path.append(path_i)
        return path
