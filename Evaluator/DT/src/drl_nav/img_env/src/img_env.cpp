...
void ImgEnv::_step_robot(comn_pkg::StepEnv::Request &req, comn_pkg::StepEnv::Response &res){
    for (int i = 0; i < req.robots.size(); i++)
    {
        ...
        if (robot_msg.alive == true)
        {
            ...
            robots_[i].trajectory = robot_msg.trajectory;
...
void ImgEnv::show_gui()
{
        ...
        {   
            ...
            for (int p = 0; p < robots_[i].trajectory.size(); p++)
            {
                tf::Transform tf_target_world = robots_[i].tf_target_world_;
                tf::Vector3 point_in_target(robots_[i].trajectory[p].x, robots_[i].trajectory[p].y, 0.0);
                tf::Vector3 point_in_world = tf_target_world * point_in_target;
                Point2i point;
                EnvMap_maps_.visual_map_.world2map(Point2d(point_in_world.x(), point_in_world.y()), point);
                point_list.push_back(point);
            }
            Point2i begin;
            EnvMap_maps_.visual_map_.world2map(Point2d(robots_[i].robot_pose_.x, robots_[i].robot_pose_.y), begin);
            point_list.insert(point_list.begin(), begin);
            for (int p = 0; p < point_list.size()-1; p++){
                line(visual_image, Point(int(resize_time * point_list[p].y), int(resize_time * point_list[p].x)),
                 Point(int(resize_time * point_list[p+1].y), int(resize_time * point_list[p+1].x)), Vec3b(0,0,255), 2);
                circle(visual_image, Point(int(resize_time * point_list[p].y), int(resize_time * point_list[p].x)), int(0.1/view_resolution_*resize_time), Vec3b(0, 0, 255), -1);
            }
            Point2i end;
