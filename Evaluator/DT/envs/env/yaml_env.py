...
class ImageEnv(gym.Env):
    ...
    def _step_req(self, actions: List[ContinuousAction], **kwargs):
        for i in range(self.robot_total):
            action: ContinuousAction = actions[i]
            path = kwargs.get('path', np.zeros((self.robot_total, 1, 2), dtype=float))[i]
            if self.dones[i] == 0:
                ...
                self.step_req.robots[i].trajectory = []
                for j in range(path.shape[0]):
                    point = Point()
                    point.x, point.y, point.z = path[j][0], path[j][1], 0.0
                    self.step_req.robots[i].trajectory.append(point)
            else:
    ...
    def load_envs_bag(self, bag_name):
        ...
        for topic, msg, t in bag.read_messages(topics=['init_env', 'reset_env']):
            ...
            if topic == 'reset_env':
                ...
                reset_req.ignore_obstacle = False
                self.reset_reqs.append(reset_req)

        bag.close()

    def step(self, actions: List[ContinuousAction], **kwargs):

        self._step_req(actions, **kwargs)
        try:
