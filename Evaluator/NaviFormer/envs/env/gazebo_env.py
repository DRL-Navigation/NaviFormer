...
class GazeboEnv(gym.Env):
    def __init__(self, cfg: dict):
        ...
        self.last_time = self.std_time_to_float(rospy.get_rostime())
        ...
        self.view_ped_time = self.std_time_to_float(rospy.get_rostime())
        self.tf_listener = tf.TransformListener()
    ...
    def get_state(self) -> ImageState:
        ...
        lasers = [latest_scan]
        distances.append(math.sqrt(state[0] ** 2 + state[1] ** 2))
        ...
        return ImageState(np.array(vec_states),
                        ...
                        #   np.array(nearby_ped),
                          step_ds,
                          np.array(tmp_ktypes, dtype=float),
    ...
    def std_time_to_float(self, data:Time):
        return data.secs + data.nsecs / 1e9

    def time_control(self):
        ############## Time Controller ###############
        while self.std_time_to_float(rospy.get_rostime()) - self.cur_time < self.dt:
            time.sleep(0.01)
        print("once step time:", self.std_time_to_float(rospy.get_rostime()) - self.cur_time, flush=True)
        self.cur_time = self.std_time_to_float(rospy.get_rostime())
    ...
    def _get_robot_state(self):
        ...
        if self.std_time_to_float(rospy.get_rostime()) - self.view_ped_time > 0.3:
            self.latest_ped_state, self.latest_ped_image = self._reset_ped_state()
