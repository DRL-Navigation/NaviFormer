...
class EnvPos:
    ...
    def _reset_robot_ped(self):
                ...
                while reset_init:
                        ...
                        while reset_init:
                            ...
                            if 'circle' in robo_ped_begin_poses_type[i]:
                                ...
                                if len(pose_range) == 4:
                                    ...
                                    rand_pose = _random_pose(pose_range[:2], pose_range[2:4], pose_range[4:6])
                            if i < ped_num:
                                if free_check_robo_ped(rand_pose[0], rand_pose[1], self.init_poses, d=0.5):
                                    self.init_poses[i] = rand_pose[:]
                                    reset_init = False
                                    break
                            else:
                                if free_check_robo_ped(rand_pose[0], rand_pose[1], self.init_poses, d=1.5) and \
                    ...
                    if 'range' in robo_ped_target_poses_type[i]:
                        while True:
                            ...
                            if i < ped_num:
                                if free_check_robo_ped(rand_pose[0], rand_pose[1], self.target_poses, d=0.5):
                                ...
                                if (self.init_poses[i][0] - rand_pose[0]) ** 2 + (
                                    ...
                                    and free_check_robo_ped(rand_pose[0], rand_pose[1], self.target_poses, d=1.5) and \
                                    free_check_robo_ped(rand_pose[0], rand_pose[1], self.init_poses, d=1.5) and \
        ...
        for i in range(ped_num, robo_ped_num):
