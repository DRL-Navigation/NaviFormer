...
```
sudo apt-get install libedit-dev ros-noetic-navigation
pip install pyyaml opencv-python rospkg gym pycryptodomex gnupg imageio matplotlib
...
# if you use anaconda, you should type following line:
catkin_make -DPYTHON_EXECUTABLE=/usr/bin/python3 --only-pkg-with-deps drl_nav
```
Tip: 编译些包没装导致报错，重装包之后把devel 和 build删掉之后重新编译
