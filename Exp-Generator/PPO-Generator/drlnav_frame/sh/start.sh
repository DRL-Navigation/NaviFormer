...
do
    ...
    then
        echo "nohup python ${argtotal} > ${log_dir}/env-${INDEX_NUM}.log 2>&1 & echo $! > ${pid_file}"
        nohup python ${argtotal} > ${log_dir}/env-${INDEX_NUM}.log 2>&1 & echo $! > ${pid_file}
    else
